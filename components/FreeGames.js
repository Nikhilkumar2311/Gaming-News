                  ...
                  <div className="relative bg-white rounded-lg shadow-md overflow-hidden border border-gray-100 h-64">
                    ...
                    <div className="absolute inset-0 opacity-80">
                      <Image
                        ...
                        priority={firstRowPosts.indexOf(post) < 2}
                      />
                    ...
                    <div className="relative p-4 flex flex-col justify-end h-full">
                      <span
                        className="inline-flex items-center backdrop-blur-sm bg-opacity-60 text-white uppercase text-xs sm:text-sm px-2 py-1 rounded self-start"
                        style={{ width: 'fit-content', whiteSpace: 'nowrap' }}
                      >
                        Free
                      </span>
                      <h3 className="font-bold mt-2 text-base sm:text-lg line-clamp-2 text-white">
                        {post.title}
                      </h3>
                      {(post.date || post.read) && (
                        <p className="text-white mt-2 text-xs sm:text-sm flex flex-wrap items-center gap-4">
                          {post.date && (
              ...
              {recentPosts.slice(0, 3).map((post) => (
                <Link key={post.id} href={`/post/${post.slug}`} className="block group">
                  <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-100 flex flex-row items-center">
                    {/* Image */}
                    <div className="w-1/3 h-20 sm:h-24 flex items-center justify-center">
                      <Image
                        src={post.image}
                        alt={post.title}
                        width={96}
                    ...
                    <div className="w-2/3 p-3 sm:p-4">
                      <span
                        className="inline-flex items-center bg-white border border-[#B8C1CD] text-[#6D757F] uppercase text-xs px-2 py-1 rounded"
                        style={{ width: 'fit-content', whiteSpace: 'nowrap' }}
                      >
                        {post.categoryName}
                      </span>
